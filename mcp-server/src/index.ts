...
function detectDiagramKeyword(diagram: string): string {
...
server.registerTool(
  ...
  {
    description:
      "Run the Mermaid parser on a diagram string and return syntax validation results. " +
    ...
    inputSchema: z.object({
      diagram: z
        .string()
        .describe(
          "The raw Mermaid diagram text (no fenced code block markers). Example: 'flowchart LR\\n  A --> B'"
        ),
    }),
  },
  async ({ diagram }: { diagram: string }) => {
    ...
    try {
      ...
      await parse(parserType as any, diagram);
...
server.registerTool(
  ...
  {
    description:
      "Run pylint on a Python file and return findings. Installs pylint automatically if not present.",
    inputSchema: z.object({
      file_path: z
        .string()
  ...
  async ({ file_path }: { file_path: string }) => {
    ...
    try {
      ...
      if (err && typeof err === "object" && "stdout" in err && "stderr" in err) {
        ...
        stderr = (err as { stdout: string; stderr: string }).stderr ?? "";
      } else {
        return {
          content: [
            {
    ...
    return {
      content: [{ type: "text" as const, text: output || "pylint produced no output." }],
    };
  }
);
